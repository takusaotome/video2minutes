...
class MinutesTask(BaseModel):
    ...
    def update_step_status(self, step_name: ProcessingStepName, status: ProcessingStepStatus, progress: int = 0, error_message: Optional[str] = None):
        """ステップのステータスを更新"""
        for step in self.steps:
            if step.name == step_name:
                ...
                elif status in [ProcessingStepStatus.COMPLETED, ProcessingStepStatus.FAILED]:
