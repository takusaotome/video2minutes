...
class FileHandler:
    ...
    def validate_video_file(file: UploadFile) -> None:
        ...
        if file_ext not in settings.allowed_video_extensions:
            logger.warning(f"サポートされていないファイル形式: {file_ext} - {file.filename}")
            raise HTTPException(
                status_code=400,
        ...
        if hasattr(file, 'size') and file.size and file.size > settings.max_file_size:
            ...
            logger.warning(f"ファイルサイズ超過: {file.filename} ({file_size_gb:.2f}GB > {max_size_gb:.1f}GB)")
            raise HTTPException(
                status_code=413,
    ...
    async def save_uploaded_file(file: UploadFile, task_id: str) -> tuple[str, int]:
        ...
        try:
            async with aiofiles.open(file_path, 'wb') as f:
                while chunk := await file.read(settings.upload_chunk_size):  # 設定可能なチャンクサイズ
                    file_size += len(chunk)
                    await f.write(chunk)
                    ...
                    if file_size > settings.max_file_size:
                        ...
                        max_size_gb = settings.max_file_size / (1024 * 1024 * 1024)
                        logger.warning(f"アップロード中にファイルサイズ超過: {file.filename} ({file_size_gb:.2f}GB > {max_size_gb:.1f}GB)")
                        raise HTTPException(
                            status_code=413,
