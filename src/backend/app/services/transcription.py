...
class TranscriptionService(LoggerMixin):
    ...
    async def _transcribe_single_file(self, audio_file_path: str) -> str:
        ...
        try:
            ...
            async with aiofiles.open(audio_file_path, 'rb') as audio_file:
                ...
                self.logger.info(f"Whisper API呼び出し開始 - モデル: {settings.whisper_model}, 言語: {settings.whisper_language}")
            ...
            self.logger.error(f"文字起こしエラー: {audio_file_path} - {str(e)}", exc_info=True)
    ...
    async def _transcribe_chunked_audio(self, chunks_dir: str) -> str:
        ...
        try:
            ...
            for i, chunk_file in enumerate(chunk_files):
                self.logger.info(f"チャンク {i+1}/{len(chunk_files)} 処理中: {os.path.basename(chunk_file)}")
            ...
            self.logger.info(f"分割音声文字起こし完了: 総文字数={total_length}, チャンク数={len(chunk_files)}")
            ...
            self.logger.error(f"分割音声処理エラー: {chunks_dir} - {str(e)}", exc_info=True)
            raise RuntimeError(f"分割音声の文字起こし中にエラーが発生しました: {str(e)}")
    ...
    async def transcribe_with_timestamps(self, audio_file_path: str) -> dict:
        ...
        try:
            ...
            raise RuntimeError(f"タイムスタンプ付き文字起こし中にエラーが発生しました: {str(e)}")
