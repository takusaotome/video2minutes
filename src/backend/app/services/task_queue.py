...
class AsyncTaskQueue(LoggerMixin):
    ...
    async def add_task(self, task_id: str, func: Callable, *args, **kwargs) -> str:
        ...
        self.logger.info(
            f"タスクをキューに追加: {task_id} (キューID: {queued_task.id})"
        )
    ...
    async def _worker(self, worker_name: str):
        ...
        while not self._shutdown:
            try:
                ...
                self.logger.error(
                    f"ワーカーエラー {worker_name}: {str(e)}", exc_info=True
                )
                # ワーカーは継続
    ...
    async def _execute_task(self, worker_name: str, queued_task: QueuedTask):
        ...
        try:
            ...
            duration = (
                queued_task.completed_at - queued_task.started_at
            ).total_seconds()
            ...
            self.logger.error(
                f"タスク実行失敗: {task_id} ({duration:.2f}秒) - {str(e)}",
                exc_info=True,
            )
            ...
            if len(self.completed_tasks) > 100:
                oldest_tasks = sorted(
                    self.completed_tasks.values(), key=lambda t: t.completed_at
                )
