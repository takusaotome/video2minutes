...
class VideoProcessor(LoggerMixin):
    ...
    async def extract_audio(self, task_id: str) -> str:
        ...
        try:
            ...
            self.logger.info(f"動画時間: {duration:.1f}秒, 使用ビットレート: {bitrate}kbps")
            ...
            self.logger.info(f"音声抽出完了: {task_id} - {audio_path} ({file_size_mb:.2f}MB)")
            
            # ファイルサイズが25MBを超える場合は分割処理を実行
            if file_size > 25 * 1024 * 1024:
                self.logger.warning(f"ファイルサイズが制限を超過 ({file_size_mb:.2f}MB) - 分割処理を実行")
    ...
    async def _run_ffmpeg_extract_mp3(self, input_path: str, output_path: str, bitrate: int) -> None:
    ...
    def _calculate_max_bitrate(self, duration_seconds: float, target_size_mb: float) -> int:
    ...
    async def _split_audio_file(self, audio_path: str, task_id: str) -> str:
        ...
        try:
            ...
            file_size = os.path.getsize(audio_path)
            target_chunk_size = (settings.audio_max_file_size_mb - 2) * 1024 * 1024  # 2MB安全マージン
            chunk_duration = (duration * target_chunk_size) / file_size
            chunk_duration = min(chunk_duration, settings.audio_chunk_duration_max)  # 設定から最大時間を取得
            
            self.logger.info(f"分割設定: 総時間={duration:.1f}秒, チャンク時間={chunk_duration:.1f}秒")
            ...
            while start_time < duration:
                ...
                # 分割コマンドを実行
                await self._split_audio_chunk(audio_path, chunk_path, start_time, chunk_duration)
    ...
    async def _split_audio_chunk(self, input_path: str, output_path: str, start_time: float, duration: float) -> None:
        """音声の一部を抽出"""
