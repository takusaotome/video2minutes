...
class TestErrorHandling:
    ...
    def test_file_handler_file_size_error(self):
        ...
        mock_file.size = 1024 * 1024 * 1024  # 1GB (上限超過)
    ...
    async def test_video_processor_ffmpeg_error(self):
        ...
        with patch('app.services.video_processor.FileHandler') as mock_handler:
            ...
            with patch.object(processor, '_run_ffmpeg_extract', new_callable=AsyncMock) as mock_ffmpeg:
    ...
    async def test_video_processor_output_file_creation_error(self):
        ...
        with patch('app.services.video_processor.FileHandler') as mock_handler:
            ...
            with patch.object(processor, '_run_ffmpeg_extract', new_callable=AsyncMock):
    ...
    async def test_minutes_generator_summary_error(self):
        ...
        mock_client = Mock()
        mock_client.chat.completions.create = AsyncMock(side_effect=Exception("Summary API Error"))
...
class TestNetworkErrorHandling:
    ...
    async def test_transcription_network_timeout(self):
        ...
        import asyncio
        mock_client = Mock()
        mock_client.audio.transcriptions.create = AsyncMock(side_effect=asyncio.TimeoutError("Timeout"))
    ...
    async def test_minutes_generation_network_error(self):
        ...
        import openai
        mock_client = Mock()
        mock_client.chat.completions.create = AsyncMock(side_effect=openai.APIConnectionError("Connection failed"))
