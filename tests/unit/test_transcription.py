import pytest
from unittest.mock import patch, Mock, AsyncMock, mock_open, MagicMock
...
class TestTranscriptionService:
    ...
    @pytest.mark.asyncio
    async def test_transcribe_audio_success(self, transcription_service, mock_openai_client):
        """文字起こし成功テスト"""
        audio_file_path = "/path/to/audio.wav"
        expected_transcription = "これはテスト用の音声文字起こし結果です。"
        
        # OpenAI APIの戻り値をモック
        mock_openai_client.audio.transcriptions.create.return_value = expected_transcription
        ...
        with patch('aiofiles.open', mock_aiofiles_open):
            with patch.object(transcription_service, 'client', mock_openai_client):
                result = await transcription_service.transcribe_audio(audio_file_path)
    ...
    @pytest.mark.asyncio
    async def test_transcribe_audio_empty_result(self, transcription_service, mock_openai_client):
        """空の文字起こし結果のテスト"""
        audio_file_path = "/path/to/audio.wav"
        ...
        with patch('aiofiles.open', mock_aiofiles_open):
            with patch.object(transcription_service, 'client', mock_openai_client):
                with pytest.raises(ValueError) as exc_info:
                    await transcription_service.transcribe_audio(audio_file_path)
                
                assert "文字起こし結果が空です" in str(exc_info.value)
    
    @pytest.mark.asyncio
    async def test_transcribe_audio_whitespace_only_result(self, transcription_service, mock_openai_client):
        """空白のみの文字起こし結果のテスト"""
        audio_file_path = "/path/to/audio.wav"
        ...
        with patch('aiofiles.open', mock_open(read_data=mock_audio_data)):
            with patch.object(transcription_service, 'client', mock_openai_client):
                with pytest.raises(ValueError) as exc_info:
                    await transcription_service.transcribe_audio(audio_file_path)
    ...
    @pytest.mark.asyncio
    async def test_transcribe_audio_api_error(self, transcription_service, mock_openai_client):
        """OpenAI APIエラーのテスト"""
        audio_file_path = "/path/to/audio.wav"
    ...
    @pytest.mark.asyncio
    async def test_transcribe_with_timestamps_success(self, transcription_service, mock_openai_client):
        """タイムスタンプ付き文字起こし成功テスト"""
        audio_file_path = "/path/to/audio.wav"
        expected_response = {
        ...
        with patch('aiofiles.open', mock_open(read_data=mock_audio_data)):
            with patch.object(transcription_service, 'client', mock_openai_client):
                result = await transcription_service.transcribe_with_timestamps(audio_file_path)
    ...
    @pytest.mark.asyncio
    async def test_transcribe_with_timestamps_error(self, transcription_service, mock_openai_client):
        """タイムスタンプ付き文字起こしエラーテスト"""
        audio_file_path = "/path/to/audio.wav"
        
        # API呼び出しでエラーを発生させる
        mock_openai_client.audio.transcriptions.create.side_effect = Exception("Timestamp API Error")
        ...
        with patch('aiofiles.open', mock_open(read_data=mock_audio_data)):
            with patch.object(transcription_service, 'client', mock_openai_client):
                with pytest.raises(RuntimeError) as exc_info:
                    await transcription_service.transcribe_with_timestamps(audio_file_path)
                
                assert "タイムスタンプ付き文字起こし中にエラーが発生しました" in str(exc_info.value)
    ...
    def test_format_transcription_with_paragraphs_short_text(self, transcription_service):
        """短いテキストの段落整形テスト"""
    ...
    def test_format_transcription_with_paragraphs_empty_text(self, transcription_service):
        """空のテキストの段落整形テスト"""
        input_text = ""
    ...
    def test_format_transcription_with_paragraphs_no_periods(self, transcription_service):
        """句点がないテキストの段落整形テスト"""
        input_text = "句点がないテキストです そのまま返されるはずです"
        
        result = transcription_service.format_transcription_with_paragraphs(input_text)
        
        # 空の結果になることを確認（句点で分割できないため）
        assert result == ""
