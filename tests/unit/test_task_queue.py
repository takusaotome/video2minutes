...
from app.services.task_queue import (
    AsyncTaskQueue, QueuedTask, TaskQueueStatus, 
    get_task_queue, initialize_task_queue, shutdown_task_queue
)
...
class TestAsyncTaskQueue:
    ...
    def test_get_queue_status(self, task_queue):
        ...
        expected_keys = ["queue_size", "running_tasks", "completed_tasks", 
                        "max_concurrent", "workers", "shutdown"]
...
class TestGlobalTaskQueueFunctions:
    ...
    def test_get_task_queue_singleton(self):
        ...
        with patch('app.services.task_queue.settings') as mock_settings:
...
class TestTaskQueueIntegration:
    ...
    async def test_concurrent_task_execution(self):
        ...
        while asyncio.get_event_loop().time() - start_time < max_wait:
            completed_count = sum(
                1 for qid in queue_ids 
                if task_queue.get_task_status(qid) and 
                task_queue.get_task_status(qid)["status"] == TaskQueueStatus.COMPLETED
            )
    ...
    async def test_error_handling_in_workers(self):
        ...
        while asyncio.get_event_loop().time() - start_time < max_wait:
            ...
            if (error_status and error_status["status"] == TaskQueueStatus.FAILED and
                success_status and success_status["status"] == TaskQueueStatus.COMPLETED):
                break
