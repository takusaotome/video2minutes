...
from app.models import MinutesTask, TaskStatus, ProcessingStepName, ProcessingStepStatus
...
class TestMinutesEndpoints:
    ...
    def test_upload_video_success(self, client):
        ...
        with patch('app.api.endpoints.minutes.FileHandler') as mock_file_handler:
            ...
            mock_file_handler.save_uploaded_file = AsyncMock(return_value=("/path/to/file.mp4", 1024000))
    ...
    def test_upload_video_validation_error(self, client):
        ...
        with patch('app.api.endpoints.minutes.FileHandler') as mock_file_handler:
            ...
            # 無効なファイル
            files = {
                "file": ("test_file.txt", b"not a video", "text/plain")
            }
    ...
    def test_get_all_tasks_with_data(self, client, sample_task):
        ...
        with patch('app.api.endpoints.minutes.tasks_store', {sample_task.task_id: sample_task}):
            ...
            assert response_data["tasks"][0]["video_filename"] == sample_task.video_filename
...
class TestProcessVideoTask:
    ...
    async def test_process_video_task_success(self, sample_task):
        ...
        with patch('app.api.endpoints.minutes.VideoProcessor') as mock_video_processor:
            with patch('app.api.endpoints.minutes.TranscriptionService') as mock_transcription:
                with patch('app.api.endpoints.minutes.MinutesGeneratorService', create=True) as mock_minutes:
                    with patch('app.api.endpoints.minutes.FileHandler') as mock_file_handler:
                        with patch('app.api.endpoints.minutes.broadcast_progress_update') as mock_broadcast:
                            with patch('app.api.endpoints.minutes.broadcast_task_completed') as mock_completed:
                                
                                # サービスのモック設定
                                mock_video_instance = mock_video_processor.return_value
                                mock_video_instance.extract_audio = AsyncMock(return_value="/path/to/audio.wav")
                                
                                mock_transcription_instance = mock_transcription.return_value
                                mock_transcription_instance.transcribe_audio = AsyncMock(return_value="文字起こし結果")
                                
                                mock_minutes_instance = mock_minutes.return_value
                                mock_minutes_instance.generate_minutes = AsyncMock(return_value="議事録結果")
                                
                                # タスクストアをモック
                                tasks_store = {sample_task.task_id: sample_task}
                                
                                with patch('app.api.endpoints.minutes.tasks_store', tasks_store):
                                    # 関数をインポートして実行
    ...
    async def test_process_video_task_error(self, sample_task):
        ...
        with patch('app.api.endpoints.minutes.VideoProcessor') as mock_video_processor:
            with patch('app.api.endpoints.minutes.FileHandler') as mock_file_handler:
                with patch('app.api.endpoints.minutes.broadcast_progress_update') as mock_broadcast:
                    with patch('app.api.endpoints.minutes.broadcast_task_failed') as mock_failed:
                        
                        # 音声抽出でエラーを発生させる
                        mock_video_instance = mock_video_processor.return_value
                        mock_video_instance.extract_audio = AsyncMock(side_effect=Exception("音声抽出エラー"))
...
class TestBroadcastFunctions:
    ...
    async def test_broadcast_progress_update(self, sample_task):
        ...
        with patch('app.api.endpoints.minutes.websocket_connections', websocket_connections):
