...
class TestVideoProcessor:
            ...
            with patch.object(video_processor, '_run_ffmpeg_extract_mp3', new_callable=AsyncMock) as mock_ffmpeg:
                with patch('os.path.exists', return_value=True):
                    with patch('os.path.getsize', return_value=1024*1024):  # 1MB
                        ...
                        mock_file_handler.get_audio_path.assert_called_once_with(task_id)
    ...
    @pytest.mark.asyncio
    async def test_extract_audio_video_file_not_found(self, video_processor, mock_file_handler):
        """動画ファイルが見つからない場合のテスト"""
    ...
    @pytest.mark.asyncio
    async def test_extract_audio_output_file_not_created(self, video_processor, mock_file_handler):
        """出力ファイルが作成されない場合のテスト"""
        ...
        with patch.object(video_processor, 'get_video_info') as mock_get_info:
            ...
            with patch.object(video_processor, '_run_ffmpeg_extract_mp3', new_callable=AsyncMock):
    ...
    async def test_run_ffmpeg_extract_failure(self, video_processor):
        ...
        # ffmpegエラーをシミュレート
        mock_process = Mock()
        mock_process.communicate = AsyncMock(return_value=(b"", b"ffmpeg error occurred"))
    ...
    async def test_extract_audio_with_real_directories(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                with patch.object(video_processor, '_run_ffmpeg_extract', new_callable=AsyncMock):
                    # 出力ファイルが作成されたことをシミュレート
                    audio_path = os.path.join(settings.temp_dir, f"{task_id}.wav")
