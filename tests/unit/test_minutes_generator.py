...
class TestMinutesGeneratorService:
    ...
    @pytest.mark.asyncio
    async def test_generate_minutes_success(self, minutes_service, mock_openai_client):
        """議事録生成成功テスト"""
        transcription = "これはテスト用の文字起こし結果です。会議の内容について話し合いました。"
        ...
        with patch.object(minutes_service, 'client', mock_openai_client):
            ...
            assert call_args['model'] == settings.gpt_model
            assert call_args['temperature'] == 0.2
            assert len(call_args['messages']) == 1  # user message only
    
    @pytest.mark.asyncio
    async def test_generate_minutes_empty_transcription(self, minutes_service, mock_openai_client):
        """空の文字起こしでの議事録生成テスト"""
    ...
    @pytest.mark.asyncio
    async def test_generate_minutes_api_error(self, minutes_service, mock_openai_client):
        """OpenAI APIエラーのテスト"""
    ...
    @pytest.mark.asyncio
    async def test_generate_minutes_empty_response(self, minutes_service, mock_openai_client):
        """空のレスポンスのテスト"""
        transcription = "テスト用の文字起こし結果"
    ...
    def test_build_prompt(self, minutes_service):
        ...
        result = minutes_service._build_prompt(transcript, meeting_name, date, attendees)
...
class TestMinutesGeneratorServiceConfiguration:
    ...
    @pytest.mark.asyncio
    async def test_generate_minutes_uses_correct_settings(self):
